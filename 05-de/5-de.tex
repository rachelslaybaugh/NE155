...
\begin{center}
{\bf NE 155, S21\\
Diffusion Equation \\ February 4, 2021}
\end{center}
...
Then we can simplify the scattering term with the assumption (which is often true) that 
\ifprintanswers
$\Sigma_s(\vOmega' \rightarrow \vOmega)$ is \textbf{azimuthally symmetric}, meaning it only depends on the angle of the scattering cosine $\mu =\vOmega' \cdot \vOmega$.  
\else
  \\\vspace*{1em}\\
\fi
...
\begin{align*}
\phi_1(\vec{r}_s, t) &= \phi_2(\vec{r}_s, t) \\
\vec{J}_1(\vec{r}_s, t) &= \vec{J}_2(\vec{r}_s, t)
\end{align*}
