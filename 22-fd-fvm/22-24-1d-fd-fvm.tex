...
where $x_{i-1/2} < \xi < {i+1/2}$ and in this case $h=(x_{i+1/2}-x_{i-1/2})$.\\
----------
%
\[f'(x_{i+1/2}) - f'(x_{i-1/2}) = (x_{i+1/2} - x_{i+1/2})r(x_i)\:.\]
%
We can now use differencing schemes to represent the derivatives on the left. We now consider the value centered at the half point and use the integer points on either side:
%
\begin{center}
\begin{tikzpicture}
