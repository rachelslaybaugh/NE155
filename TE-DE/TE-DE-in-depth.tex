...
Now, we integrate the TE over $v(E)$:
\begin{align}
\frac{1}{v_0}\frac{\partial \psi(\vec{r}, \vOmega, t)}{\partial t} &+ 
\vOmega \cdot \nabla \psi(\vec{r}, \vOmega, t) +
\Sigma_t \psi(\vec{r}, \vOmega, t) = \nonumber\\
%
& \int_{4\pi} d\vOmega' \Sigma_s(\vOmega' \cdot \vOmega) \psi(\vec{r}, \vOmega', t)  + Q(\vec{r}, \vOmega, t)
\end{align}
...
%
& \int_{4\pi} d\vOmega' \Sigma_s(\vOmega' \cdot \vOmega) \psi(x, \vOmega', t)  + Q(x, \vOmega, t)
\end{align}

% -------- 
Here, our integrals over angle are performed over $2\pi$, \textbf{assuming azimuthal symmetry} (no angular dependence on $\phi$):
...
\begin{align}
\psi(x, \mu, t) &= \int_0^{2\pi} \psi(x, \vOmega, t) d\phi = 2\pi \psi(x, \vOmega, t) \\
Q(x, \mu, t) &= \int_0^{2\pi} Q(x, \vOmega, t) d\phi = 2\pi Q(x, \vOmega, t)
\end{align}
\end{minipage}
...
\Sigma_t \psi(x, \mu, t) = \nonumber\\
%
& \int_0^{2\pi} d\phi \int_{4\pi} d\vOmega' \Sigma_s(\vOmega' \cdot \vOmega) \frac{\psi(x, \mu', t)}{2\pi}  + 2\pi Q(x, \mu, t)
\end{align}
