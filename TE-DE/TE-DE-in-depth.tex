...
\begin{align}
\frac{1}{v_0}\frac{\partial \psi(\vec{r}, \vOmega, t)}{\partial t} &+ 
...
$d\vOmega = \sin(\theta) d\theta	d\phi = d\mu d\phi$
...
\frac{1}{4\pi} \nu \Sigma_f \int_{4\pi} d\vOmega' \psi(x,  \vOmega, t) + s(x, \vOmega, t)%+ Q(x, \vOmega, t)
\end{align}
...
\item the diffusion equation reduces this to 5-D space
\end{itemize}

The diffusion equation is derived by starting with the transport equation and integrating over all angles. We'll use the one-group version for simplicity, but this assumption is not needed for the derivation. 

% Identities
We will use the following identities in our derivation:
\begin{align} 
\int_{4\pi} d\vOmega &= 4\pi \\
%
\int_{4\pi} d\vOmega \vOmega &= 0 \qquad \text{because }\vOmega\text{ is an odd function} \\
%
\int_{4\pi} d\vOmega \vOmega \vOmega &= \frac{4\pi}{3}\ve{I} \qquad \ve{I}\text{ is the identity tensor, which means} \\
%
...
Also, recall the definition of scalar flux and (net) current:
\begin{align}
...
%
\begin{align}
\int_{4\pi} d\vOmega &\biggl[\frac{1}{v_0}\frac{\partial \psi(\vec{r}, \vOmega, t)}{\partial t} + 
\vOmega \cdot \nabla \psi(\vec{r}, \vOmega, t) +
\Sigma_t \psi(\vec{r}, \vOmega, t) = \nonumber\\
%
& \frac{\nu \Sigma_f}{4\pi}\phi(\vec{r},t) +  \int_{4\pi} d\vOmega' \Sigma_s(\vOmega' \cdot \vOmega) \psi(\vec{r}, \vOmega', t)  + \frac{Q(\vec{r}, t)}{4\pi}\biggr]
\end{align}
%
...
\begin{equation}
{1}{v_0}\frac{\partial}{\partial t} \int_{4\pi} d\vOmega  \psi(\vec{r}, \vOmega, t) = \boxed{\frac{1}{v_0}\frac{\partial}{\partial t}\phi(\vec{r}, t)}
\end{equation}
%%%%
...
\end{equation}
%%%%
\textbf{4.} Includes isotropic assumption
\begin{equation}
\frac{\nu \Sigma_f}{4\pi}\int_{4\pi} d\vOmega \phi(\vec{r},t) = \boxed{\nu \Sigma_f \phi(\vec{r}, t)}
\end{equation}
%%%%
\textbf{6.} Includes isotropic assumption
\begin{equation}
\int_{4\pi} d\vOmega \frac{Q(\vec{r}, t)}{4\pi} = \boxed{Q(\vec{r}, t)}
\end{equation}
%%%%
\textbf{5.} Further investigate the scattering cross section%, including the azimuthal symmetry assumption

%Expand the scattering cross section in Legendre Polynomials (these for a sequence of orthogonal polynomials):
...
%\Sigma_s(\vOmega' \cdot \vOmega) &=  \frac{1}{4\pi}\bigl( \Sigma_{s0} + 3\mu_0 \Sigma_{s0} \bigr) \qquad l=1 \text{ linearly isotropic}
%\end{align}
%
...
\begin{equation}
\int_{4\pi} d\vOmega \int_{4\pi} d\vOmega' \Sigma_s(\vOmega' \cdot \vOmega) \psi(\vec{r}, \vOmega', t) = \boxed{\Sigma_s \phi(\vec{r}, t)}
\end{equation}
