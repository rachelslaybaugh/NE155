...
\vspace*{2em}
When we put all of the terms together we get the \textbf{neutron continuity equation}. You can see that we have one equation, but two unknowns (technically 4 unknowns b/c $\vec{J}$ has 3 spatial components)
\begin{equation}
...
For the scattering term we will do some mathematical manipulation. There are several ways to do this, but this one makes the most sense to me.

Expand the scattering cross section in Legendre Polynomials (these are a sequence of orthogonal polynomials):
...
\Sigma_s(\vOmega' \cdot \vOmega) &=  \frac{1}{4\pi}\bigl( \Sigma_{s0} + 3\vOmega' \cdot \vOmega \Sigma_{s0} \bigr) \qquad l=1 \text{ linearly isotropic}
\end{align}
...
%
\begin{align}
\frac{1}{4\pi} \int_{4\pi} &d\vOmega \int_{4\pi} d\vOmega' \vOmega' \bigl[\Sigma_{s0} + 3\vOmega' \cdot \vOmega \Sigma_{s0} \bigr] \psi(\vec{r}, \vOmega', t) = \nonumber\\
&\frac{1}{4\pi} \int_{4\pi} d\vOmega \int_{4\pi} d\vOmega' \vOmega' \Sigma_{s0}\psi(\vec{r}, \vOmega', t) +
\frac{1}{4\pi} \int_{4\pi} d\vOmega \vOmega' \cdot \vOmega \int_{4\pi} d\vOmega' 3 \vOmega' \Sigma_{s1}\psi(\vec{r}, \vOmega', t) \\
%
&= 0 + \frac{3}{4\pi} \underbrace{\int_{4\pi} d\vOmega \vOmega \vOmega}_{\frac{4\pi}{3}} \Sigma_{s1} \vec{J}(\vec{r},t) \\
%
