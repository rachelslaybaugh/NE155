...
\]    
%    \begin{align*}
%    f(t)dt &= e^{-\lambda t} dt \:, \quad t > 0 \\
%    %
%    F(t) &= \int_{-\infty}^t f(t')dt' = \int_0^t f(t') dt' = \biggl[- \frac{e^{-\lambda t'}}{\lambda} \biggr]_0^t = \frac{1}{\lambda} \bigl(1 - e^{-\lambda t}\bigr)\\
%    %
%    F(\infty) &= \frac{1}{\lambda}\\
%    \end{align*}
%    \pause
%    \begin{align*}
%    p(t) &= \lambda f(t) = \lambda e^{-\lambda t} dt \:, \quad t > 0 \\
%    %
%    P(t) &= \int_{-\infty}^t p(t')dt' = \int_0^t \lambda f(t') dt' = \bigl[e^{-\lambda t'} \bigr]_0^t = 1 - e^{-\lambda t}\\
%    %
%    P(\infty) &= 1
