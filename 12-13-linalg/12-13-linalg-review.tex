...
\begin{document}
\begin{center}
{\bf NE 155, Class 12, 13, \&14, S17 \\
 Vectors and Matrices: Feb 13-17, 2017}
\end{center}
...
\[||\hat{x}^{(k+1)} - \vec{x}||_p = \mu ||\hat{x}^{(k)} - \vec{x}||_p^q \rightarrow ||e_{k+1}||_p = \mu||e_{k+1}||_p^p\:.
\]
Consider
\begin{compactitem}
\item If $q=1$ and $0<\mu<1$,  $||e_{k+1}||_p =\mu || e_k ||_p < || e_k ||_p$, then
  \begin{compactitem}
  \item if $\mu=1$, the convergence is sublinear
  \item if $0<\mu<1$, the convergence is linear with the rate of convergence of $\mu$.
  \item if $\mu=0$, the convergence is superlinear
  \end{compactitem}
