...
\begin{document}
\begin{center}
{\bf NE 155, Class 3, S15 \\
Types of Equations in the Engineering Fields \\ January 26, 2015}
\end{center}
...
Boundary conditions:
\begin{enumerate}
\item Initial Value Problem (\textbf{IVP}): if y and its derivatives are given at one end of the domain/interval (e.g.\ time 0 if there's time or x 0 if there's only space, etc.)
...
This has
\begin{itemize}
\item Neumann BCs if $\gamma = 0$ (specifies the values that the \textit{derivative} of a solution is to take on the boundary of the domain)
\item Dirichet BCs: if $\frac{d\phi}{dx} = 0$ (specifies the values that a \textit{solution} is to take on the boundary of the domain)
\item Mixed BCs if [$\gamma \ne 0$ and $\frac{d\phi}{dx} = 0$ at $x = \alpha$ and $\gamma = 0$] and [$\frac{d\phi}{dx} \ne 0$ at $x = \beta$] (the solution is required to satisfy a Dirichlet or a Neumann boundary condition in a mutually exclusive way on disjoint parts of the boundary)
...
%-------------------------------------------------------------
\vspace*{1em}
\underline{Classification}:%http://en.wikipedia.org/wiki/Partial_differential_equation#Classification
...
\end{align*}
There's no $B$ term, and $C$ is negative so $-4AC > 0$
  \begin{itemize}
