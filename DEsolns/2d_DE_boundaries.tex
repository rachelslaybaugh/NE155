...
%
\begin{align}
- \int_{S_3} d\vec{S} \:D(\vec{r})\frac{\partial}{\partial \hat{n}}\phi(\vec{r}) &= \boxed{\frac{\phi_{0,j} - \phi_{0,j-1}}{\epsilon_{j}} \biggl(\frac{D_{1,j} \delta_{1}}{2}\biggr)} \nonumber \\ 
%
- \int_{S_6} d\vec{S} \:D(\vec{r})\frac{\partial}{\partial \hat{n}}\phi(\vec{r}) &= \boxed{\frac{\phi_{0,j+1} - \phi_{0,j}}{\epsilon_{j+1}} \biggl(\frac{D_{1,j+1} \delta_{1}}{2}\biggr)} \nonumber
\end{align}

The absorption and streaming terms now become:
\begin{align}
\int \int dx dy\:\Sigma_a(x,y) \phi(x,y) &= \boxed{\phi_{0,j}\bigl(\Sigma_{a,1,j} V_{1,j} + \Sigma_{a,1,j+1} V_{1,j+1} \bigr) } \nonumber \\
&\equiv \Sigma^*_{a,0j}\:, \nonumber \\
%
\int \int dx dy \: \:S(x,y) &= \boxed{S_{1,j} V_{1,j} + S_{1,j+1} V_{1,j+1}}\nonumber \\
&\equiv S^*_{0j}\:. \nonumber
\end{align}
...
%
a_{0,j-1}^{*,0j} &= -\frac{D_{i+1,j} \delta_{1}}{2 \epsilon_{j}}  \nonumber \\
%
a_{0,j+1}^{*,0j} &= -\frac{D_{i+1,j+1} \delta_{1}}{2 \epsilon_{j+1}}  \nonumber \\
%
...
%
\begin{align}
- \int_{S_2} d\vec{S} \:D(\vec{r})\frac{\partial}{\partial \hat{n}}\phi(\vec{r}) &= \boxed{\frac{\phi_{n,j} - \phi_{n,j-1}}{\epsilon_{j}} \biggl(\frac{D_{n,j} \delta_{n}}{2}\biggr)} \nonumber \\ 
%
- \int_{S_7} d\vec{S} \:D(\vec{r})\frac{\partial}{\partial \hat{n}}\phi(\vec{r}) &= \boxed{\frac{\phi_{n,j+1} - \phi_{n,j}}{\epsilon_{j+1}} \biggl(\frac{D_{n,j+1} \delta_{n}}{2}\biggr)} \nonumber
\end{align}

The absorption and streaming terms now become:
\begin{align}
\int \int dx dy\:\Sigma_a(x,y) \phi(x,y) &= \boxed{\phi_{n,j}\bigl(\Sigma_{a,n,j} V_{n,j} + \Sigma_{a,n,j+1} V_{n,j+1} \bigr) } \nonumber \\
&\equiv \Sigma^*_{a,nj}\:, \nonumber \\
%
\int \int dx dy \: \:S(x,y) &= \boxed{S_{n,j} V_{n,j} + S_{n,j+1} V_{n,j+1}}\nonumber \\
&\equiv S^*_{nj}\:. \nonumber
\end{align}
...
%
\[\boxed{a_{1,j}^{*,nj}\phi_{1,j} + a_{0,j-1}^{*,nj}\phi_{0,j-1} + a_{0,j+1}^{*,nj}\phi_{0,j+1} +  a_{0,j}^{*,nj}\phi_{0,j} = S^*_{nj}} \]
%
Recall: the lower index is the cell to which you are coupling, and the upper index is which cell you are in. The coefficients how change to become
\begin{align}
a_{1,j}^{*,nj} &= -\frac{D_{1,j} \epsilon_{j} + D_{1,j+1} \epsilon_{j+1}}{2 \delta_{1}}  \nonumber \\
%
a_{0,j-1}^{*,nj} &= -\frac{D_{i+1,j} \delta_{n}}{2 \epsilon_{j}}  \nonumber \\
%
a_{0,j+1}^{*,nj} &= -\frac{D_{i+1,j+1} \delta_{n}}{2 \epsilon_{j+1}}  \nonumber \\
%
a_{0,j}^{*,nj} &= \Sigma^*_{a,nj} - \bigl(a_{1,j}^{nj} + a_{0,j-1}^{nj} + a_{0,j+1}^{nj} \bigr)
 \:.\nonumber 
\end{align}
