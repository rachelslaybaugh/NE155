...
%
- \int_{S_6} d\vec{S} \:D(\vec{r})\frac{\partial}{\partial \hat{n}}\phi(\vec{r}) &= \boxed{\frac{\phi_{0,j+1} - \phi_{0,j}}{\epsilon_{j+1}} \biggl(\frac{D_{1,j} \delta_{1}}{2}\biggr)} \nonumber
\end{align}
...
% surrounding grid and markers
\draw plot[mark=*, mark options={fill=black}] (0,2) node[left] {$(m-1,j)$};
\draw (0,2) -- plot[mark=*, mark options={fill=black}] (4,2) node[right] {$(m,j)$};
\draw (4,0)-- plot[mark=*, mark options={fill=black}] (4,-2) node[below] {$(m,j-1)$};
\draw (4,4)-- plot[mark=*, mark options={fill=black}] (4,6) node[above] {$(m,j+1)$};
% arrows
\draw[<->](0.5,2) -- (0.5,6);
